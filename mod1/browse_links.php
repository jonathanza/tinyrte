...
$LANG->includeLLFile('EXT:tinyrte/mod1/locallang_browse_links.xml');
...
class tinyrte_template extends template {
...
class localPageTree extends t3lib_browseTree {
	...
	function printTree($treeArr='')	{
		...
		foreach($treeArr as $k => $v)	{
			...
			$cEbullet = $this->ext_isLinkable($v['row']['doktype'],$v['row']['uid'],$GLOBALS['SOBE']->act) ?
						'<a href="#" onclick="'.htmlspecialchars($aOnClick).'"><img'.t3lib_iconWorks::skinImg($BACK_PATH,'gfx/ol/arrowbullet.gif','width="18" height="16"').' alt="" title="'.$v['row']["title"].'" /></a>' :
						'';
			$out.='
				<tr>
					<td nowrap="nowrap" width="210px" title="'.$v['row']["title"].'">'.
					$v['HTML'].
	 ...
	 * @return	boolean
	 */
	function ext_isLinkable($doktype,$uid,$act="")	{
		if (($uid && $doktype<199) || ($act=="news" && $doktype==254))	{
			return true;
		}
	}
...
class localFolderTree extends t3lib_folderTree {
	...
	function printTree($treeArr='')	{
		...
		$titleLen=25; //intval($GLOBALS['BE_USER']->uc['titleLen']);
		...
		foreach($treeArr as $k => $v)	{
			...
			$cEbullet = $this->ext_isLinkable($v['row']) ? '<a href="#" onclick="'.htmlspecialchars($aOnClick).'"><img'.t3lib_iconWorks::skinImg($BACK_PATH,'gfx/ol/arrowbullet.gif','width="18" height="16"').' alt="" title="'.$v['row']['title'].'"/></a>' : '';

				// Put table row with folder together:
			$out.='
				<tr class="'.$bgColorClass.'">
					<td width="210" nowrap="nowrap">'.$v['HTML'].$this->wrapTitle(t3lib_div::fixed_lgd_cs($v['row']['title'],$titleLen),$v['row']).'</td>
					'.$arrCol.'
					<td>'.$cEbullet.'</td>
				</tr>';
		}
...
class SC_browse_links {
	...
	function init() {
		...
		$this->newsPID = t3lib_div::_GP('news');
		...
		if ($this->mode=='wizard')	{	// Functions used, if the link selector is in wizard mode (= TCEforms fields)
			...
			$JScode.='
				...
				function checkReference()	{	//
					if (popWin && popWin.document && popWin.document.'.$this->P['formName'].' && popWin.document.'.$this->P['formName'].'["'.$this->P['itemName'].'"] )	{
						return popWin.document.'.$this->P['formName'].'["'.$this->P['itemName'].'"];
					} else if (popWin && popWin.document && popWin.document.'.$this->P['formName'].' ) {
						return popWin.document.'.$this->P['formName'].';
					} else {
						self.close();
					}
				}
				function updateValueInMainForm(input)	{	//
					...
					if (field)	{
						...
						popWin.setTheValue("link",input,"'.$_GET["act"].'");'.$update.'
					}
				}
			';
				...
				if (parent.window.opener && parent.window.opener.setFormValueFromBrowseWin)	{
					...
					parent.close();
                        ...
                        function setPos(e) {
                            ...
                            var tempWidth = document.getElementById("left").style.width;
                        ...
                        function getPos(e){
                            if( mouseButtonPos == "down" ) {
                                ...
                                newWidth = ((newWidth > 430)? 430: newWidth);
                                document.cookie = "browsewidth="+newWidth;
                                setWidth(newWidth);
                            }
                        }
...
'.$this->doc->wrapScriptTags($JScode);
	...
	function main_rte($wiz=0)	{
		...
		if (in_array('page',$allowedItems)) {
			...
			$menuDef['page']['url'] = '#';
			$menuDef['page']['addParams'] = 'onclick="jumpToUrl(\'?act=page&news='.$this->newsPID.'\');return false;"';
		}
		if (in_array('file',$allowedItems)){
			$menuDef['file']['isActive'] = $this->act=='file';
			$menuDef['file']['label'] = $LANG->getLL('file',1);
			$menuDef['file']['url'] = '#';
			$menuDef['file']['addParams'] = 'onclick="jumpToUrl(\'?act=file&news='.$this->newsPID.'\');return false;"';
		}
		if (in_array('url',$allowedItems)) {
			$menuDef['url']['isActive'] = $this->act=='url';
			$menuDef['url']['label'] = $LANG->getLL('extUrl',1);
			$menuDef['url']['url'] = '#';
			$menuDef['url']['addParams'] = 'onclick="jumpToUrl(\'?act=url&news='.$this->newsPID.'\');return false;"';
		}
		if (in_array('mail',$allowedItems)) {
			$menuDef['mail']['isActive'] = $this->act=='mail';
			$menuDef['mail']['label'] = $LANG->getLL('email',1);
			$menuDef['mail']['url'] = '#';
			$menuDef['mail']['addParams'] = 'onclick="jumpToUrl(\'?act=mail&news='.$this->newsPID.'\');return false;"';
		}
		if (in_array('news',$allowedItems) && t3lib_extmgm::isLoaded("tt_news")) {
			$menuDef['news']['isActive'] = $this->act=='news';
			$menuDef['news']['label'] = "News";
			$menuDef['news']['url'] = '#';
			if($this->newsPID) {
				$menuDef['news']['addParams'] = 'onclick="jumpToUrl(\'?act=news&news='.$this->newsPID.'\');return false;"';
			}
			else {
				$menuDef['news']['addParams'] = 'onclick="alert(\'You have to set a Page-Id for singleview (RTE.default.newsSinglePid) in TSconfig to use Links to news.\');return false;"';
			}
		}
		...
		// add extra container
		$content .= '<div class="panel_wrapper" style="height: 350px;">';
		...
		switch($this->act)	{
					...
					<fieldset>
					  <legend>'.$GLOBALS['LANG']->getLL('emailAddress',1).'</legend>
						...
						<table border="0" cellpadding="2" cellspacing="1" id="typo3-linkMail">
							<tr>
								<td><input type="text" name="lemail"'.$this->doc->formWidth(20).' value="'.htmlspecialchars($this->curUrlInfo['value']?$this->curUrlInfo['info']:'').'" /> '.
					...
					<fieldset>
					  <legend>URL</legend>
						...
						<table border="0" cellpadding="2" cellspacing="1" id="typo3-linkURL">
							<tr>
								<td><input type="text" name="lurl"'.$this->doc->formWidth(20).' value="'.htmlspecialchars($this->curUrlInfo['value']?$this->curUrlInfo['info']:'http://').'" /> '.
			...
			case 'file':
				...
				$files='<fieldset><legend>'.$GLOBALS['LANG']->getLL('files').'</legend></legend><DIV id="right" STYLE="width: 265px; height: 240px; overflow-x:hidden ; overflow-y:scroll;"><table><tr><td>'.$files.'</td></tr></table></div></fieldset>';
                                $content.='
                        <!--
			...
			-->
					<table border="0" cellpadding="0" cellspacing="0" height="125">
						<tr>
							<td valign="top" style="width: 277px;"><fieldset><legend>'.$GLOBALS['LANG']->getLL('folderTree').'</legend><DIV id="left" STYLE="width: 265px; height: 240px; overflow-x:hidden ; overflow-y:scroll;"><table><tr><td>'.$tree.'</td></tr></table></div></fieldset></td>
							<td style="width: 10px;"><img src="../sizer.gif" width="10" onmousedown="setPos(event)" title="<- || ->"></td>
							<td valign="top" style="width: 277px;">'.$files.'</td>
						</tr>
					</table>
					';
			...
			case 'news':
				...
				$cElements ='<fieldset><legend>'.$GLOBALS['LANG']->getLL('contentElements').'</legend><DIV id="right" STYLE="width: 265px; height: 240px; overflow-x:hidden ; overflow-y:scroll;"><table><tr><td>'.$cElements.'</td></tr></table></div></fieldset>';
		...
		if ($this->act!='mail')	{
			...
			-->
				<form action="" name="ltargetform" id="ltargetform">
					<table border="0" cellpadding="2" cellspacing="0" id="typo3-linkTarget">
						<tr>
			...
			$content.='
				   <img src="clear.gif" height="8" width="100%"><fieldset>
	...
	function expandPage()	{
		...
		if ($expPageId && t3lib_div::testInt($expPageId) && $BE_USER->isInWebMount($expPageId))	{
			...
			if($GLOBALS['SOBE']->act=="news") {
				$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
								'uid,title as header,hidden,starttime,endtime,fe_group,bodytext',
								'tt_news',
								'pid='.intval($expPageId).t3lib_BEfunc::deleteClause('tt_news')." AND sys_language_uid=0",
								'',
								'crdate'
							);
			}
			...
			while($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res))	{
				...
				if (($this->curUrlInfo['act']=='page' || $this->curUrlInfo['act']=='news') && $this->curUrlInfo['cElement']==$row['uid'])	{
				...
				$t=strlen(htmlspecialchars(t3lib_div::fixed_lgd_cs($row['header'],$titleLen)))>35 ? '&nbsp;'.substr(htmlspecialchars(t3lib_div::fixed_lgd_cs($row['header'],$titleLen)),0,35).'...' : '&nbsp;'.htmlspecialchars(t3lib_div::fixed_lgd_cs($row['header'],$titleLen));


				$out.='<img'.t3lib_iconWorks::skinImg($BACK_PATH,'gfx/ol/join'.($c==$cc?'bottom':'').'.gif','width="18" height="16"').' alt="" />'.
						$arrCol.
						'<a href="#" onclick="return link_typo3Page(\''.$expPageId.'\',\'#'.$row['uid'].'\');" title="'.htmlspecialchars(t3lib_div::fixed_lgd_cs($row['header'],$titleLen)).'">'.
						$icon.
						$t.
						'</a><br />';
	...
	function printCurrentUrl($str)	{
		...
		return '
			...
			</fieldset><img src="clear.gif" height="8" width="100%">';
	}
...
// Include extension?
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/tinyrte/mod1/browse_links.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/tinyrte/mod1/browse_links.php']);
}
