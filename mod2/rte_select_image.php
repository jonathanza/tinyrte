...
$LANG->includeLLFile('EXT:tinyrte/mod2/locallang_rte_select_image.php');
//$LANG->includeLLFile('EXT:tinyrte/mod2/locallang_rte_select_image.xml');
...
<script language="javascript" type="text/javascript">
	...
	function init()	{
		...
		*/
		...
		';
			...
			// Starting content:
		$this->content="";
		$this->content.=str_replace("<body", '<body onmousemove="getPos(event)" onmouseup="mouseButtonPos=\'up\'"'.(($_COOKIE["browsewidth"] && $_GET["act"]!="upload") ? ' onLoad="setWidth('.$_COOKIE["browsewidth"].')"' : ""), $this->doc->startPage("TYPO3 Imagebrowser"));
	}
	...
	function main()	{
		...
		if (in_array("magic",$this->allowedItems)) $menu.='<li '.($this->act=="magic"?$bgcolorA:$bgcolor).' title="'.str_replace('"', "'",$LANG->getLL("magicImage_msg")).'"><span><a href="#" onClick="jumpToUrl(\'?act=magic\');return false;">'.$LANG->getLL("magicImage").'</span></a></li>';
		if (in_array("plain",$this->allowedItems)) $menu.='<li '.($this->act=="plain"?$bgcolorA:$bgcolor).' title="'.str_replace('"', "'",$LANG->getLL("plainImage_msg")).'"><span><a href="#" onClick="jumpToUrl(\'?act=plain\');return false;">'.$LANG->getLL("plainImage").'</span></a></li>';
		$menu.='<li '.($this->act=="upload"?$bgcolorA:$bgcolor).' title="'.str_replace('"', "'",$LANG->getLL("uploadImage_msg")).'"><span><a href="#" onClick="jumpToUrl(\'?act=upload\');return false;">'.$LANG->getLL("uploadImage").'</span></a></li>';
		$menu.='
				</ul>
			</div>';

		$this->content.=$menu;
		$this->content.='<div class="panel_wrapper" style="height: 350px;">';
		...
		if ($this->act!="upload")	{
			...
			$files = '<fieldset><legend>'.$GLOBALS['LANG']->getLL('images').'</legend><DIV id="right" STYLE="width: 265px; height: 240px; overflow-x:hidden ; overflow-y:scroll;"><table width="100%" border=0 cellpadding=0 cellspacing=0><tr><td>'.$files.'</td></tr></table></div></fieldset>';
			$this->content.= '<table border="0" cellpadding="0" cellspacing="0" width="445" height="170">
			<tr>
				<td valign="top" style="width: 277px;"><fieldset><legend>'.$LANG->getLL("folderTree").'</legend><DIV id="left" STYLE="width: 265px; height: 240px; overflow-x:hidden ; overflow-y:scroll;"><table><tr><td>'.$tree.'</td></tr></table></div></fieldset></td>
			...
			if (is_array($files))	{
				...
				while(list(,$filepath)=each($files))	{
					...
					$lines[]='<tr><td>'.(
						...
						'</td></tr>';
					$lines[]='<tr><td><img src="clear.gif" width=1 height=3></td></tr>';
				}
				$out.='<table width="100%" border=0 cellpadding=0 cellspacing=1>'.implode("",$lines).'</table>';
			}
		}
		return $out;
	}
	...
	//	debug($path);
		...
		$code.='
			<input type="Hidden" name="redirect" value="'.t3lib_extMgm::extRelPath('tinyrte').'mod2/rte_select_image.php?act=magic&expandFolder='.rawurlencode($path).'&RTEtsConfigParams='.rawurlencode(t3lib_div::_GP("RTEtsConfigParams")).'">
			<img src="clear.gif" height="8" width="100%">
			<input type="Submit" name="submit" value="'.$LANG->sL("LLL:EXT:lang/locallang_core.php:file_upload.php.submit").'"><img src="clear.gif" height="8" width="100%">
			<div id="c-override">
...
// Include extension?
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/tinyrte/mod2/rte_select_image.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/tinyrte/mod2/rte_select_image.php']);
}
